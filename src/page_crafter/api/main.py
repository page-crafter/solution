...
from page_crafter.api.routers import (
    auth,
    chat,
    confluence,
    jobs,
...
from page_crafter.shared.db.init import create_database_schema

DEFAULT_FRONTEND_DIST_DIR = Path("frontend/dist")
PACKAGE_FRONTEND_DIST_DIR = Path(__file__).resolve().parent / "static"


def frontend_dist_dir() -> Path:
    """Return the built Vue app directory for this runtime."""
    configured_dir = os.getenv("FRONTEND_DIST_DIR")
    if configured_dir:
        return Path(configured_dir).resolve()

    if (PACKAGE_FRONTEND_DIST_DIR / "index.html").is_file():
        return PACKAGE_FRONTEND_DIST_DIR
