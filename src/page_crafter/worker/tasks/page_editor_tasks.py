...
from page_crafter.shared.confluence.storage_markdown import build_preserved_storage_map
from page_crafter.shared.db.session import SessionLocal
from page_crafter.shared.models.confluence import ConfluencePage
from page_crafter.shared.models.page_editor import DraftArtifact, PageEditRun, PageProposal
from page_crafter.worker.celery_app import celery_app
from page_crafter.worker.confluence.preview import (
    preview_has_rendered_content,
    render_storage_preview,
)
from page_crafter.worker.confluence.publish import publish_generated_page
...
from page_crafter.worker.page_editor.diffing import build_markdown_diff, build_storage_diff
from page_crafter.worker.page_editor.generate_markdown import generate_markdown_draft
from page_crafter.worker.page_editor.proposals import propose_markdown_update
from page_crafter.worker.rag.lightrag import sync_pages_to_lightrag
...
@celery_app.task(name="page_crafter.render_draft")
...
@celery_app.task(name="page_crafter.convert_markdown")
def convert_markdown(run_id: str) -> None:
    """Convert a Markdown draft to Confluence Storage XHTML."""
    task_name = "page_crafter.convert_markdown"
    with SessionLocal() as session:
        result = get_run_and_page(session, run_id)
        if result is None:
            mark_task_finished(
